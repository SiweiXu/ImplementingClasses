...
class Point(object):
    """Points are dots that move, reflect, and attach."""
...
def run_test_get_distance_from_start():
    ...
    class Point(object):
        ...
        def get_distance_from_start(self):
            import math
            d1 = self.x - self.startx
            d2 = self.y - self.starty
            d = math.sqrt(d1 ** 2 + d2 ** 2)
...
def run_test_get_distance_traveled():
    ...
    class Point(object):
        ...
        def move_by(self, c, d):
            ...
            self.y += d
            self.count += 1
            self.distance_by += math.sqrt(c ** 2 + d ** 2)
        ...
        def move_to(self, a, b):
            self.distance_to += self.get_distance_from(Point(a, b))
            self.x = a
            self.y = b
            self.count += 1
        ...
        def get_distance_traveled(self):
            ...
            distance_traveled = self.distance_by + self.distance_to
    ...
    class Point(object):
        ...
        def closer_to(self, p1, p2):
            if self.get_distance_from(p1) > self.get_distance_from(p2):
                return p2
            if self.get_distance_from(p2) >= self.get_distance_from(p1):
                return p1
...
def run_test_halfway_to():
    ...
    class Point(object):
        ...
        def halfway_to(self, p2):
            newpoint = Point((self.x + p2.x) / 2, (self.y + p2.y) / 2)
            return newpoint
